...
The registry file is YAML matching the packaged JSON Schema at `nwave_ai/outcomes/schema.json` (draft-07). Each entry is one element of the top-level `outcomes:` list.
...
- **JSON Schema** — `nwave_ai/outcomes/schema.json`.
